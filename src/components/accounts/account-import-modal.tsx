...
export function AccountImportModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const parse = async (file: File) => {
    setError(""); setSummary("");
    try {
      const buffer = await file.arrayBuffer(); const workbook = XLSX.read(buffer, { type: "array", raw: false }); const sheet = workbook.Sheets[workbook.SheetNames[0] ?? ""];
      ...
      const mapped = source.map((record, index) => {
