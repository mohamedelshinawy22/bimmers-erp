...
function previewStateLabel(row: InvoiceImportPreview["rows"][number], mode: ImportMode) {
  ...
  const channels = row.paymentChannels.length ? `قنوات: ${row.paymentChannels.map((channel) => `${channel.name} (${channel.amount.toLocaleString("ar-EG")})`).join(" + ")}` : "";
  if (row.accountStatus === "CASH_FALLBACK") return ["عميل نقدي افتراضي", channels].filter(Boolean).join(" — ");
  if (row.accountStatus === "AUTO_CREATE") return ["سيُنشأ الحساب تلقائياً", channels].filter(Boolean).join(" — ");
  ...
  return channels ? `مطابق — ${channels}` : "مطابق";
}
