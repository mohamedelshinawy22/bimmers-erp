...
function isIgnoredSpreadsheetRow(row: Record<string, unknown>, headers: string[]) {
  ...
  const normalizedHeaders = new Set(headers.map((header) => header.trim().toLocaleLowerCase("ar-EG")));
  const repeatedHeader = values.length >= 2 && values.every((value) => normalizedHeaders.has(value.toLocaleLowerCase("ar-EG")));
...
export function ExcelImportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const fileChange = async (file?: File) => {
    ...
    const parsedRows = json.map((row, index) => ({ ...row, __sourceRowNumber: index + 2 }));
    ...
    setRows(cleanRows); setHeaders(fileHeaders); setPrunedRows(parsedRows.length - cleanRows.length); setMapping({}); setSkipInvalidRows(true); setMessage(""); setStep(2);
  };
