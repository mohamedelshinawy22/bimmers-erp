...
type PreviewRow = PhysicalCountRow & { status: "MATCHED" | "UNMATCHED" | "AMBIGUOUS" | "INVALID"; message: string; partId: string | null; matchedBy: "OEM_BRAND" | "OEM_NAME" | "OEM" | "NAME_BRAND" | "NAME_CATEGORY" | "NAME" | null; bookQuantity: number | null; delta: number | null; partName: string | null; partOemNumber: string | null };
...
export function StocktakeReconciliationModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const downloadUnmatchedReport = () => {
    if (!preview) return;
    const rows = preview.rows.filter((row) => row.status !== "MATCHED");
    const columns = ["رقم الصف", "OEM / كود", "اسم الصنف", "الكمية الفعلية", "الحالة", "سبب الاستبعاد"];
    const escape = (value: unknown) => `"${String(value ?? "").replaceAll('"', '""')}"`;
    const csv = [columns, ...rows.map((row) => [row.sourceRowNumber, row.oemNumber, row.nameAr, row.actualQuantity ?? "", row.status, row.message])].map((row) => row.map(escape).join(",")).join("\n");
    const anchor = document.createElement("a"); anchor.href = URL.createObjectURL(new Blob([`\uFEFF${csv}`], { type: "text/csv;charset=utf-8" })); anchor.download = "stocktake-unmatched-rows.csv"; anchor.click(); URL.revokeObjectURL(anchor.href);
  };
