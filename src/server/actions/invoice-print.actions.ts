...
export async function getInvoicePrintDataAction(invoiceId: string): Promise<ActionResult<InvoicePrintData>> {
  try {
    ...
    const historicLedger = invoice.type === "PURCHASE" && (invoice.accountBalanceBefore === null || invoice.accountBalanceAfter === null)
      ? await getAccountDetailedLedger(invoice.account.id)
      : null;
    const linkedLedgerRows = historicLedger?.rows.filter((row) => row.invoiceId === invoice.id) ?? [];
    const purchaseInvoiceRow = linkedLedgerRows.find((row) => row.type === "PURCHASE");
    const balanceBefore = invoice.accountBalanceBefore ?? (purchaseInvoiceRow ? purchaseInvoiceRow.runningBalance - invoice.grandTotal : null);
    const balanceAfter = invoice.accountBalanceAfter ?? linkedLedgerRows.at(-1)?.runningBalance ?? invoice.account.currentBalance;
    ...
    return ok({
      ...
      invoice: {
        ...
        cashierName: invoice.user.fullName, accountBalanceBefore: balanceBefore, accountBalanceAfter: balanceAfter, verificationUrl, qrPayload,
      },
