...
function emptyDailyMovementReport(input: DailyMovementReportInput) {
  const operations = ([['sales', 'بيع'], ['saleReturns', 'مرتجع بيع'], ['purchases', 'شراء'], ['purchaseReturns', 'مرتجع شراء'], ['receipts', 'قبض'], ['payments', 'صرف'], ['stocktakes', 'جرد مخزن'], ['branchTransfers', 'تحويل لمخزن'], ['adjustments', 'تسوية مخزن']] as Array<[string, string]>).map(([key, label]) => ({ key, label, count: 0, total: 0, paid: 0, remaining: 0 }));
  const treasurySummary = ([['cashSales', 'بيع نقدي'], ['cashSaleRefunds', 'مرتجع بيع نقدي'], ['cashPurchases', 'شراء نقدي'], ['cashPurchaseReturns', 'مرتجع شراء نقدي'], ['receipts', 'قبض'], ['payments', 'صرف'], ['invoiceExpenses', 'مصروف فواتير'], ['opening', 'رصيد سابق'], ['net', 'صافي حركة الخزينة'], ['closing', 'الرصيد النهائي']] as Array<[string, string]>).map(([key, label]) => ({ key, label, amount: 0 }));
  return { scope: { fromDate: input.fromDate.toISOString(), toDate: input.toDate.toISOString(), operatorId: input.operatorId || null, warehouseName: input.warehouseName || null, treasuryIds: input.treasuryIds }, filterOptions: { users: [], treasuries: [], warehouses: [] }, operations, treasurySummary, drillDowns: { sales: [], saleReturns: [], purchases: [], purchaseReturns: [], receipts: [], payments: [], stocktakes: [], adjustments: [], treasuryMovements: [] } };
}
...
export async function getDailyMovementReportAction(raw: DailyMovementReportInput) {
  try {
    ...
    const [invoices, ledger, stockMoves, priorLedger, users, treasuries, warehouseRows] = await Promise.all([
      ...
      tenant.prisma.treasuryTransaction.findMany({
      ...
      tenant.prisma.stockMovement.findMany({
      ...
      tenant.prisma.user.findMany({ where: { isActive: true }, orderBy: { fullName: "asc" }, select: { id: true, fullName: true, username: true } }),
      tenant.prisma.treasury.findMany({ where: { isActive: true }, orderBy: { name: "asc" }, select: { id: true, name: true } }),
    ...
    const invoiceRows = (type: InvoiceKind) => invoices.filter((invoice) => invoice.type === type).map((invoice) => {
      ...
      return {
        id: invoice.id, documentId: invoice.id, reference: invoice.invoiceNumber, at: invoice.createdAt?.toISOString() ?? new Date(0).toISOString(), party: invoice.account?.name ?? "حساب غير متاح",
        ...
        treasury: invoice.treasury?.name ?? "—", warehouse: warehouseName ?? "—", user: invoice.user ? `${invoice.user.fullName} (@${invoice.user.username})` : "مستخدم غير معروف", source: type,
      };
    });
    ...
    const treasuryRows = ledger.map((row) => ({
      id: row.id, documentId: row.invoiceId, reference: row.invoice?.invoiceNumber ?? row.transactionNumber, at: row.createdAt?.toISOString() ?? new Date(0).toISOString(), party: row.account?.name ?? "—", description: row.description ?? "",
      total: signedLedger(row), paid: signedLedger(row), remaining: 0, treasury: row.treasury?.name ?? "خزينة غير متاحة", warehouse: "—", user: operatorNames.get(row.createdByUser) ?? "مستخدم غير معروف",
    ...
    const stocktakeRows = stockMoves.filter((row) => row.reason === "STOCKTAKE").map((row) => ({
      id: row.id, documentId: row.invoice?.id ?? null, reference: row.invoice?.invoiceNumber ?? `STK-${row.id.slice(0, 8)}`, at: row.createdAt?.toISOString() ?? new Date(0).toISOString(), party: "—", description: row.note ?? `${row.part?.nameAr ?? "صنف غير متاح"} (${row.part?.oemNumber ?? "—"})`,
      total: row.quantityDelta * n(row.unitCost), paid: 0, remaining: 0, treasury: "—", warehouse: row.part?.binLocation?.warehouseName ?? "غير محدد", user: row.performedBy ? `${row.performedBy.fullName} (@${row.performedBy.username})` : "مستخدم غير معروف", source: "جرد مخزن", quantityDelta: row.quantityDelta,
    }));
    const adjustmentRows = stockMoves.filter((row) => row.reason === "MANUAL_ADJUSTMENT").map((row) => ({
      id: row.id, documentId: row.invoice?.id ?? null, reference: row.invoice?.invoiceNumber ?? `ADJ-${row.id.slice(0, 8)}`, at: row.createdAt?.toISOString() ?? new Date(0).toISOString(), party: "—", description: row.note ?? `${row.part?.nameAr ?? "صنف غير متاح"} (${row.part?.oemNumber ?? "—"})`,
      total: row.quantityDelta * n(row.unitCost), paid: 0, remaining: 0, treasury: "—", warehouse: row.part?.binLocation?.warehouseName ?? "غير محدد", user: row.performedBy ? `${row.performedBy.fullName} (@${row.performedBy.username})` : "مستخدم غير معروف", source: "تسوية مخزن", quantityDelta: row.quantityDelta,
    }));
    ...
    return ok(serializeData({
      ...
      console.error("Daily movement query failed; returning empty report", { tenantId: tenant.user.tenantId, error });
      return ok(serializeData(emptyDailyMovementReport(input)));
    }
    });
  } catch (error) {
