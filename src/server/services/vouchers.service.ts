...
export async function getVoucherRegister(filters: VoucherRegisterFilters = {}): Promise<VoucherRegisterData> {
...
export async function getVoucherFilterTreasuries() {
  return prisma.treasury.findMany({ where: { isActive: true }, orderBy: { name: "asc" }, select: { id: true, name: true, currentBalance: true, isDefault: true } }).then((rows) => rows.map((row) => ({ ...row, currentBalance: num(row.currentBalance) })));
}

export async function getVoucherAccounts() {
  return prisma.account.findMany({ where: { isActive: true }, orderBy: [{ name: "asc" }], take: 1_000, select: { id: true, name: true, accountNumber: true, type: true, phone: true } });
}

export async function getOpenInvoicesForVouchers() {
  return prisma.invoice.findMany({ where: { isVoided: false, remainingAmount: { gt: 0 }, type: { in: ["SALE", "PURCHASE"] } }, orderBy: { createdAt: "desc" }, take: 500, select: { id: true, invoiceNumber: true, type: true, accountId: true, account: { select: { name: true, accountNumber: true } }, remainingAmount: true } }).then((rows) => rows.map((row) => ({ id: row.id, invoiceNumber: row.invoiceNumber, type: row.type, accountId: row.accountId, accountName: row.account.name, accountNumber: row.account.accountNumber, remainingAmount: num(row.remainingAmount) })));
}
