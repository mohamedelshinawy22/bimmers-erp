...
import { insertRestoreChunk } from "@/server/services/chunked-restore.service";
...
function errorMessage(error: unknown) {
  return error instanceof Error ? error.message : "حدث خطأ غير متوقع أثناء إدراج دفعة الاستعادة.";
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json() as { restoreToken?: unknown; table?: unknown; rows?: unknown };
    const result = await insertRestoreChunk({ actor, restoreToken: String(body.restoreToken ?? ""), table: String(body.table ?? ""), rows: body.rows });
