...
const tools: ForgeTool[] = [
  ...
  { type: "function", function: { name: "queryProducts", description: "البحث الحي عن الأصناف برقم OEM أو الاسم أو الباركود أو الفئة مع الرصيد والأسعار المسموح بها.", parameters: { type: "object", properties: { query: { type: "string" }, chassis: { type: "string" }, engine: { type: "string" }, lowStockOnly: { type: "boolean" } }, additionalProperties: false } } },
...
const safeMessages = (value: unknown): ForgeMessage[] => {
  ...
  try {
    ...
    const result = await tenant.run(async () => {
      ...
      for (let round = 0; round < 4; round += 1) {
        const completion = await invokeLLM({ messages: conversation, tools, toolChoice: "auto", maxTokens: 900 });
        const assistant = completion.choices?.[0]?.message;
        if (!assistant) return { reply: "تعذر تكوين إجابة من خدمة المساعد حالياً." };
        conversation.push(assistant);
        if (!assistant.tool_calls?.length) return { reply: assistant.content?.trim() || "لم أجد إجابة نصية للطلب." };
      ...
      return { reply: "تعذر إكمال الاستعلام في الوقت المحدد. أعد صياغة السؤال بشكل أقصر." };
    });
