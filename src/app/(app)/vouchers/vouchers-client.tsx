...
import { ArrowDownLeft, ArrowUpRight, ChevronDown, Download, Eye, FileUp, MoreHorizontal, Printer, RotateCcw, Search, Trash2, X } from "lucide-react";
...
import { VoucherImportModal } from "@/components/vouchers/voucher-import-modal";
import { CombinedVoucherImportModal } from "@/components/vouchers/combined-voucher-import-modal";
import { AccountCombobox, type VoucherAccountOption } from "@/components/common/account-combobox";
...
export function VouchersClient({ rows, summary, filters, treasuries, accounts, openInvoices, company, permissions, initialAction }: Props) {
  ...
  const [importType, setImportType] = useState<VoucherType | null>(null);
  ...
  return <div className="space-y-4" dir="rtl">
    <header className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><div className="rounded-xl border border-bmw-blue/30 bg-bmw-blue/10 p-2.5 text-bmw-blue"><ArrowDownLeft size={22} /></div><div><h1 className="text-lg font-bold text-white">سجل وإدارة السندات والتحصيلات</h1><p className="text-xs text-bmw-muted">إدارة سندات القبض والصرف، دورة الإلغاء والاستعادة، والطباعة المحاسبية.</p></div></div><div className="flex flex-wrap gap-2">{permissions.canTransact ? <><Button variant="success" onClick={() => setCreateType("RECEIPT")}><ArrowDownLeft size={16} /> سند قبض جديد <span className="mr-1 text-[10px] opacity-70">F8</span></Button><Button variant="danger" onClick={() => setCreateType("PAYMENT")}><ArrowUpRight size={16} /> سند صرف جديد <span className="mr-1 text-[10px] opacity-70">F9</span></Button><Button variant="primary" onClick={() => setCombinedImportOpen(true)}><FileUp size={16} /> استيراد شامل (قبض + صرف)</Button><div className="relative"><Button variant="outline" onClick={() => setImportMenuOpen((open) => !open)}><FileUp size={16} /> استيراد سندات Excel <ChevronDown size={14} /></Button>{importMenuOpen ? <div className="absolute left-0 top-full z-30 mt-1.5 w-56 rounded-xl border border-bmw-cardBorder bg-bmw-carbon p-1.5 shadow-2xl"><button type="button" onClick={() => { setImportMenuOpen(false); setImportType("RECEIPT"); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs text-emerald-300 hover:bg-emerald-500/10">🟢 استيراد سندات قبض (.xlsx)</button><button type="button" onClick={() => { setImportMenuOpen(false); setImportType("PAYMENT"); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs text-rose-300 hover:bg-rose-500/10">🔴 استيراد سندات صرف (.xlsx)</button><div className="my-1 border-t border-bmw-cardBorder" /><button type="button" onClick={() => { setImportMenuOpen(false); setImportType("RECEIPT"); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs text-bmw-blue hover:bg-bmw-blue/10">📥 تحميل النموذج القياسي من المعالج</button></div> : null}</div></> : null}<Button variant="outline" onClick={exportXlsx} loading={exporting}><Download size={16} /> تصدير إكسيل</Button><Button variant="outline" onClick={printLedger}><Printer size={16} /> طباعة السجل</Button></div></header>
    ...
    {importType ? <VoucherImportModal type={importType} onClose={() => setImportType(null)} onDone={() => { setImportType(null); router.refresh(); }} /> : null}
    {combinedImportOpen ? <CombinedVoucherImportModal treasuries={treasuries} onClose={() => setCombinedImportOpen(false)} onDone={() => { setCombinedImportOpen(false); router.refresh(); }} /> : null}
...
function CreateVoucherModal({ type, treasuries, accounts, openInvoices, onClose }: { type: VoucherType; treasuries: Props["treasuries"]; accounts: Props["accounts"]; openInvoices: Props["openInvoices"]; onClose: () => void }) {
  ...
  const [voucherType, setVoucherType] = useState<VoucherType>(type); const [treasuryId, setTreasuryId] = useState(treasuries.find((item) => item.isDefault)?.id ?? treasuries[0]?.id ?? ""); const [accountId, setAccountId] = useState(""); const [invoiceId, setInvoiceId] = useState(""); const [amount, setAmount] = useState(""); const [category, setCategory] = useState("CASH"); const [description, setDescription] = useState(""); const [createdAt, setCreatedAt] = useState(asLocalInput(new Date()));
  const selectedTreasury = treasuries.find((item) => item.id === treasuryId); const selectedInvoice = openInvoices.find((item) => item.id === invoiceId); const parsedAmount = Number(amount) || 0; const relevantInvoices = openInvoices.filter((invoice) => voucherType === "RECEIPT" ? invoice.type === "SALE" : invoice.type === "PURCHASE");
