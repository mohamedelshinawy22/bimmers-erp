...
function QuickAccountModal({ onClose, onCreated }: { onClose: () => void; onCreated: (account: PosAccount) => void }) {
  ...
  const submit = () => startTransition(async () => { const result = await createQuickPosAccountAction({ name, phone, type, defaultPriceTier: "RETAIL", notes: "إنشاء سريع من نقطة البيع" }); if (!result.success) { setError(result.error); return; } onCreated({ ...result.data, vehicleCount: 0 }); });
